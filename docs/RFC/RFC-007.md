...
- Use `scripts/godot/Install-MythTagPlayPulseBridge.ps1` and `scripts/godot/Start-MythTagWithPlayPulse.ps1` to validate the addon against the local MythTag consumer project.
- The MythTag bridge is a local validation helper only; the shipped SDK remains generic and game-agnostic.
...
## Decision
Adopt the above SDK surface, queue strategy, and reliability guarantees for the Godot 4 MVP. The addon, standalone SDK tests, and MythTag validation scripts are now implemented in the repository.
